...
class Meanshifter:
    ...
    def meanshift(self,inputClass,inputFeature):
        ...
        for batch in range(batchSize):
            ...
            centers = np.zeros((200000,shape[1]))   #feature center of the class
            clsOfBlk = np.zeros((200000)).astype(np.int8)  #indicate which class a color block belong to
            #print('batch: %d'%batch)
            ...
            #################################get proper eat threshold###########################
            eatThreshold = getThreshold(output[batch])
            #################################end################################################


            #################################break big block, eat small block########################
            totalClass = eatBlk(output[batch],clsOfBlk,totalClass,centers,inputFeature[batch],eatThreshold)
            totalClass = eatBlk(output[batch],clsOfBlk,totalClass,centers,inputFeature[batch],eatThreshold)
            totalClass = eatBlk(output[batch],clsOfBlk,totalClass,centers,inputFeature[batch],eatThreshold)
            #################################end################################################
            ...
            for color in range(1,totalClass):
                if (cnt[color] != 0):
                    ...
                    clsOfBlk[currentClass] = clsOfBlk[color]
                    currentClass += 1
            #################################end################################################
            ...
            for x in range(h):
                for y in range(w):
                    if (clsOfBlk[output[batch,x,y]] == 0 and output[batch,x,y] != 0):
                        output[batch,x,y] = 0
                    inputClass[batch,x,y] = clsOfBlk[output[batch,x,y]]
        return output,inputClass
...
if (__name__ == '__main__'):
    for i in range(0,19):
        inputData = np.load('ipt/input%d.npy'%i)
        classData = np.load('fms/class%d.npy'%i)
        featureData = np.load('fms/instance%d.npy'%i)
        output,classData = Meanshifter(1.1,1e-3,1.1,500).meanshift(classData,featureData)
        for batch in range(output.shape[0]):
            output[batch] = trim_color(output[batch],classData[batch])
        print('test %d done'%i)
        np.save('myoutput/output%d'%i,output)
        np.save('myoutput/outputCls%d'%i,classData)
        visualize_colored(torch.from_numpy(inputData),output,classData,prefix='-%d'%i)
